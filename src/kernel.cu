  ...
  // Rule 3: boids try to match the speed of surrounding boids
    ...
    for (int i = 0; i < N; i++) {
        ...
        if (distance < rule1Distance) {
            rule1PerceivedCenter += pos[i];
            rule1Neighbors++;
        }

        if (distance < rule2Distance) {
            rule2Delta -= (pos[i] - pos[iSelf]);
        }
    ...
    glm::vec3 rule1Contribution = (rule1PerceivedCenter - pos[iSelf]) * rule1Scale;
    glm::vec3 rule2Contribution = rule2Delta * rule2Scale;
    glm::vec3 rule3Contribution = rule3PerceivedVelocity * rule3Scale;

    glm::vec3 newVelocity = vel[iSelf] + rule1Contribution + rule2Contribution + rule3Contribution;
    return newVelocity;
}
...
__device__ glm::ivec3 posToGridIndex3D(glm::vec3 pos, int gridResolution, glm::vec3 gridMin, float inverseCellWidth) {
...
__global__ void kernIdentifyCellStartEnd(int N, int *particleGridIndices,
  ...
  // "this index doesn't match the one before it, must be a new cell!"
    ...
    int prevGridIndex = index > 1 ? particleGridIndices[index - 1] : -1;
...
__device__ bool isValidGridCell(glm::ivec3 gridCellIndex3D, int gridResolution) {
    return gridCellIndex3D.x >= 0 && gridCellIndex3D.y >= 0 && gridCellIndex3D.z >= 0 &&
        gridCellIndex3D.x < gridResolution && gridCellIndex3D.y < gridResolution && gridCellIndex3D.z < gridResolution;
}
...
__global__ void kernUpdateVelNeighborSearchScattered(
  ...
  // - Clamp the speed change before putting the new speed in vel2
    ...
    for (int dz = deltaGridIndexMin.z; dz <= deltaGridIndexMax.z; dz++) {
        for (int dy = deltaGridIndexMin.y; dy <= deltaGridIndexMax.y; dy++) {
            for (int dx = deltaGridIndexMin.x; dx <= deltaGridIndexMax.x; dx++) {
                ...
                for (int i = gridCellStartIndices[neighborGridIndex1D]; i <= gridCellEndIndices[neighborGridIndex1D]; i++) {
                    if (i < 0 || i >= N) {
                        continue;
                    }
    ...
    glm::vec3 rule1Contribution = (rule1PerceivedCenter - pos[index]) * rule1Scale;
    glm::vec3 rule2Contribution = rule2Delta * rule2Scale;
    glm::vec3 rule3Contribution = rule3PerceivedVelocity * rule3Scale;
...
__global__ void kernUpdateVelNeighborSearchScatteredDynamicGrid(
    int N, int gridResolution, glm::vec3 gridMin,
    float inverseCellWidth, float cellWidth,
    int* gridCellStartIndices, int* gridCellEndIndices,
    int* particleArrayIndices,
...
__global__ void kernReorderPosAndVel(int N, int* particleArrayIndices, glm::vec3* pos, glm::vec3* vel, glm::vec3* coherentPos, glm::vec3* coherentVel) {
    int index = (blockIdx.x * blockDim.x) + threadIdx.x;
    if (index >= N) {
        return;
    }
    ...
    coherentVel[index] = vel[originalIndex];
}
...
__global__ void kernUpdateVelNeighborSearchCoherentDynamicGrid(
    int N, int gridResolution, glm::vec3 gridMin,
    float inverseCellWidth, float cellWidth,
    int* gridCellStartIndices, int* gridCellEndIndices,
    glm::vec3* pos, glm::vec3* vel1, glm::vec3* vel2,
...
void Boids::stepSimulationScatteredGrid(float dt) {
  ...
  // - Ping-pong buffers as needed
    ...
    checkCUDAErrorWithLine("kernUpdateVelNeighborSearchScatteredDynamicGrid failed!");
#else
...
void Boids::stepSimulationCoherentGrid(float dt) {
  ...
  // - Ping-pong buffers as needed. THIS MAY BE DIFFERENT FROM BEFORE.
    ...
    kernUpdateVelNeighborSearchCoherentDynamicGrid << <fullBlocksPerGrid, blockSize >> > (
        ...
        rule1Distance, rule2Distance, rule3Distance);
    checkCUDAErrorWithLine("kernUpdateVelNeighborSearchCoherentDynamicGrid failed!");
#else
...
void Boids::stepSimulationCoherentGridWithAudio(float dt, SongFeatures* dev_songFeatures) {
    // TODO-2.3 - start by copying Boids::stepSimulationNaiveGrid
    // Uniform Grid Neighbor search using Thrust sort on cell-coherent data.
    // In Parallel:
    // - Label each particle with its array index as well as its grid index.
